"""CLI for shared-policy PPO self-play on 2-player Hanabi."""
...
from tom.training.ippo_hanabi import HanabiPPOConfig, train  # noqa: E402
...
def main():
    ...
    ap.add_argument("--belief-coef", type=float, default=0.0,
                    help="Coefficient on belief auxiliary loss; 0.0 = vanilla baseline")
    ap.add_argument("--belief-in-policy", action="store_true",
                    help="BAD/SAD-style: concat belief output into policy/value head input")
    ...
    log_dir = args.log_dir or f"runs_hanabi/ippo_{args.env_name.replace('-', '_').lower()}"
    ...
    cfg = HanabiPPOConfig(
        ...
        belief_in_policy=args.belief_in_policy,
        hidden=args.hidden,
